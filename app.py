        ...
        with tab5:
            ...
            if (not FC_STORE.exists()) and (not OBS_STORE.exists()):
                ...
                if not FC_STORE.exists():
                    ...
                    # Robustesse: certains CSV existants stockent ces colonnes en string
                    fc_hist["time"] = pd.to_datetime(fc_hist["time"], errors="coerce", utc=True)
                    fc_hist["issued_at_utc"] = pd.to_datetime(fc_hist["issued_at_utc"], errors="coerce", utc=True)
                    fc_hist = fc_hist.dropna(subset=["time", "issued_at_utc"])
                    ...
                    if fc_hist.empty:
                        ...
                        if m.empty:
                            ...
                            # Robustesse: certains CSV peuvent stocker des valeurs en string
                            for c in [
                                "temp_raw", "rh_raw", "rain_raw",
                                "temp_obs", "rh_obs", "rain_obs",
                            ]:
                                if c in m.columns:
                                    m[c] = pd.to_numeric(m[c], errors="coerce")
                            m = m.dropna(subset=["temp_raw", "temp_obs"])
                            ...
                            try:
                                ...
                                m["rh_corr"] = m["rh_corr"].clip(40, 100)
